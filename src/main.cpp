...
// stays here until the VNC capability gets its own TU.
...
void create_editor_content(flecs::entity leaf, EditorType editor_type, flecs::entity UIElement)
{
    ...
    // Panels that have moved out of the monolith into src/panels/ modules
    // dispatch through the registry; the else-if chain below is the legacy
    // path, shrinking as panels migrate.
    if (const PanelDef* def = panels::find(editor_type)) {
        def->create_content(leaf, UIElement);
        return;
    }
    ...
    // EditorType::Holodeck moved to src/panels/holodeck_panel.cpp -- the
    // registry dispatch above handles it.
